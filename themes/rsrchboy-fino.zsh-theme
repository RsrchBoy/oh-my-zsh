...
function prompt_status() {
  ...
  symbols=''
  symbols+="$iPOWER $POWER_SUPPLY_CAPACITY%% "
  symbols+="%(1j.%{%F{cyan}%}${iJOB} .)"
  symbols+="%(!.%{%F{red}%}%B$iROOT%b )"
  symbols+="%(?..%{%F{red}%}$iFROWN )"


  #prompt_segment black default "${symbols:-$iOK}"
...
function build_prompt() {
  ...
  #prompt_segment black blue "$(rvm_prompt_info)$(rbenv_prompt_info)"
  ...
  #prompt_end
  #echo "%B»%b "
  echo " \u${CODEPOINT_OF_AWESOME_DOUBLE_ANGLE_RIGHT}"
}
